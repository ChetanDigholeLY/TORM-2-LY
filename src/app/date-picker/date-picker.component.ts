...
function customDateFormatter(date: Date | number | any): String {
  return dateFormatter.format(date).replaceAll('/', '-').replaceAll(',', ' ');
}
...
export class DatePickerComponent {
  ...
  ranges: any = {
    ...
    'Last Year': [dayjs().subtract(1, 'year').startOf('month'), dayjs()],
    All: [dayjs(), dayjs()],
  };
  ...
  updateDateRange() {
    let dates = this.datePickerRef.chosenLabel
      .split('-')
  ...
  onRangeButtonClick(event: any, key: any) {
    ...
    this.closeDatePickerEvent.emit({
      range: [new Date(dates[0]), new Date(dates[1])],
      predefinedValue: key,
    });
  }
